...
\abstract{The study of underwater structures, such as wells, cisterns and water storage systems, can be of historical and scientific significance for archeologists.  Such sites pose a challenge as access and study of them can be dangerous and infeasible for humans. 
Robots can often reach these locations safer and cause less harm, especially underwater micro-ROVs, such as the Videoray  Pro 3 GTO, which can navigate very small tunnels and openings.  
Prior work has successfully reconstructed geometric models of such underwater structures with the use of sonar measurements.  Although effective, sonar has a fairly limited resolution and many fine geometric details of the model are lost in the scanning process.  In this paper, we present a preliminary solution towards the accurate reconstruction of fine details of geometric features in the underwater setting using stereo pairs.  These fine details can then integrated into 3D models reconstructed from sonar to create a more accurate final model of the water system.  The initial rough 3D model is generated using multiple sonar scans of the walls of the site.  The fine details of the internal organic surfaces are recorded using stereoscopic imaging.
Third and finally, the rough 3D model is deformed and textured using the details obtained through stereo imaging. We present initial results and detailed methodology for future data acquisition.
}
...
%Tim, you need to add some general stereo references - as well as the color one for underwater!
