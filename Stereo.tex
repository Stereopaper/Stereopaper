...
\vspace*{-3mm}
The study of underwater structures, such as wells, cisterns, and water storage systems, can be of historical and scientific significance for archeologists. However, access to and study of such sites can be dangerous or infeasible for humans.
Underwater micro-ROVs, such as the VideoRay Pro III GTO, can often reach and navigate these locations safely while causing less harm to the site.
...
\begin{figure*}[ht!]
   ...
   \caption{The sonar data acquisiton and general model reconstruction stages of the project. Sonar data is collected in a 2D scan plane, merged into a 2D surface using FastSLAM, marching cubes, and a hole filling algorithm, and extruded into 3D.}
  \label{fig:meshgen}
 \end{figure*}
...
finish this up (bla bla bla, we cull this stuff, then stuff is sent to shaders)
%\begin{align}
%\nonumber %Left plane
...
p_{m}' = \left \{ 
\begin{array}{ll}
p_{m} + (\vec{b} - \hat{b}D(C_r))  & \text{if} \quad p_{m} \in P^{*}\\
p_{m} & \text{otherwise}
\end{array},\right.
\label{eq:displace}
\end{equation}
...
p_{m}' = \left \{ 
\begin{array}{ll}
p_{m} + \sum\limits_{\vec{b} \in B} \frac{\vec{b} - \hat{b}D(C_r)}{q} & \text{if} \quad p_{m} \in P^{*}\\
p_{m} & \text{otherwise}
\end{array},\right.
...
%TP: unfortunately, not really.  That is what HSL was supposed to do for us, but failed at...  
...
\begin{figure}[!h]
	\centering
		\subfigure[Projected Images]{\label{left}\epsfig{file = pics/noDisparity.png, width = 6.5cm}}\\
		\quad %space between images
			\subfigure[Projected Disparity maps]{\label{left}\epsfig{file = pics/mapsDisparity.png, width = 6.5cm}}\\		
