...
{Williams2000,harbor,Fairfield2006}.
%ZJW comment - lets only site one Fairfield given space
...
Related work in underwater imaging and robotics includes 3D sonar scans projectively textured with image data~\cite{Bingham:2010,stereo:Negahdaripour}, and 3D maps of underwater settings generated using vision-only SLAM~\cite{Mahon:2011,stereo:Roberson,stereo:Aqua}.    
Similarly, we deformed and textured 3D sonar scans using stereo vision algorithms.  
Our depth reconstruction from stereo images relied primarily on~\cite{stereo:zitKan} along with similar work in stereo mapping~\cite{stereo:scharsteinSzeliski}.  

%Stereo vision based SLAM algorithms have been used to generate 3D maps of underwater settings~\cite{Mahon:2011,stereo:Roberson,stereo:Aqua}. Similarly, other work has been done to texture sonar scans with image data~\cite{Bingham:2010,stereo:Negahdaripour}However, we are using stereo images to enhance models created from sonar data.  Finally, this research relies on related work in the field of stereoscopic data acquisition and disparity map reconstruction~\cite{stereo:scharsteinSzeliski}.
...
\begin{figure*}[!t]
	\vspace{-0.2cm}
		...
		\caption{To the left, a view of depth and color data texture-mapped onto part of the general mesh. The fine details added by the vertex displacements and recalculated lighting create a better appearance. Image includes inset close-up view. To the right, a photo of the setup used to measure error, as well its the corresponding disparity map.}
...
For future missions, the ROV should capture all images of the cistern wall before landing to avoid uplighting sentiment. 
...
Finally, we would like to apply learning to a small subset of disparity images in order to generalize the small detailed geometry without having to acquire disparity maps of the entire wall.% (since complete data coverage is a challenge).
...
%\newblock Three dimensional evidence grids for {SLAM} in complex underwater environments.
...
\bibitem{Bingham:2010}
Bingham, Foley, Singh, Camilli, Delaporta, Eustice, Mallios, Mindell, Roman, Sakellariou. (2010) 
\newblock Robotic tools for deep water archeology: Surveying an ancient shipwreck with an autonomous underwater vehicle
\newblock In {\em Journal of Field Robotics}.
